...
//    @Query("SELECT d.nombre,e.nombre,c.fechaCarga,c.fechaActividad,e.documento, FROM Reserva c,Actividad d,Pasajero e WHERE c.actividad_id=d.id AND c.pasajero_id=e.id ORDER BY d.nombre")
...
//    //Esta lista deberia imprimirse en un PDF
//    @Query("SELECT d.nombre,e.nombre,c.fechaCarga,c.fechaActividad,e.documento, FROM Reserva c,Actividad d,Pasajero e WHERE c.actividad_id=d.id AND c.pasajero_id=e.id AND d.nombre LIKE %:nombre% ORDER BY d.nombre")
